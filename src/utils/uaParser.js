...
const mapper = {
  rgx (ua, arrays) {
    ...
    while (i < arrays.length && !matches) {
      ...
      let props = arrays[i + 1] // odd sequence (1,3,5,..)
      j = k = 0
...
class UAParser {
  ...
  static getInstance (uastring, extensions) {
    if (this.instance instanceof this === false) {
      this.instance = new this(uastring, extensions)
    }
  ...
  constructor (uastring, extensions) {
    ...
    this.ua = uastring || ((window && window.navigator && window.navigator.userAgent) ? window.navigator.userAgent : EMPTY)
